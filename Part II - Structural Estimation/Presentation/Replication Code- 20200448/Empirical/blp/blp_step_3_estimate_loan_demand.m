...
samplelist={'loan_1','loan_2','loan_3','loan_4', 'loan_5'}; 
...
price = data(:,5)-.8; % 0.8 is the mean charge-off
share = data(:,4);
...
A=sparse([A0 dum_t(:,2:end) dum_j(:,1:end)]); %need to remove year==1

if sample_num>1
A=sparse([A0 dum_t(:,1:end) ]); % do not use bank fixed effect in subsamples for loans, because the post period cannot be identified
end 
...
x_U     = [0*ones(nrc,1)];       % upper bound (only for the nested logit, to avoid numerical problems when pho gets close to 1)
...
varq2=Dq_beta2*varcovar(2:end,2:end)*Dq_beta2';
...
varq1=Dq_beta1*varcovar(2:end,2:end)*Dq_beta1';
...
parameters{2,1}='sigma_alpha';parameters{2,2}='NA';
parameters{3,1}='q_NonBankLoan';parameters{3,2}=num2str(q_sum(1,1));
parameters{4,1}='q_BankLoan_sum';parameters{4,2}=num2str(q_sum(2,1));
parameters{5,1}='N';parameters{5,2}=num2str(num_rep_bank(1));
parameters{6,1}='semi-elasticity';parameters{6,2}=num2str(mean(OwnSemiElast_hat));
parameters{7,1}='elasticity';parameters{7,2}=num2str(mean(OwnSemiElast_hat.*price));
parameters{8,1}='marginal cost';parameters{8,2}=num2str(mean_mc_hat_g(2));
parameters{9,1}='alpha (se)';parameters{9,2}=num2str(setsls(1));
parameters{10,1}='sigma_alpha (se)';parameters{10,2}='NA';
parameters{11,1}='q_NonBankLoan (se)';parameters{11,2}=num2str(seq1);
parameters{12,1}='q_BankLoan_sum (se)';parameters{12,2}=num2str(seq2);

output_table_name=strcat(outputfolder,'\demand_',samplelist{sample_num},'.xls');
xlswrite(output_table_name, parameters)

save(samplelist{sample_num},'btsls','setsls', 'Rsq_tsls') % the deposit file compile both results in one table
end
