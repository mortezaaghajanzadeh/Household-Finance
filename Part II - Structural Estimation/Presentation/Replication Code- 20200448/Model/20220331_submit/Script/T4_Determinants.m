...
%%
Par.a1 = 0;  Par.theta = 0; Par.ksi = 0;
Par = Grid_exp(Par); solution5A =solve_model_MP(Par,'NO DMP','YES LMP');
[firm5A, simu_con5A, simu_un5A] = SimulatePanel_short(Par, solution5A, shock0);
%%
Par = Grid_exp(Par); solution6A = solve_model_noLMP(Par);
load('Data\shocks_6_500000_1_0_0'); 
[firm6A, simu_con6A, simu_un6A] = SimulatePanel_short(Par, solution6A, shock0);
%%
Par = Grid_exp(Par); solution7A =solve_model_MP(Par,'NO DMP','NO LMP');
[firm7A, simu_con7A, simu_un7A] = SimulatePanel_short(Par, solution7A, shock0);
%% Print results
...
inc6 = (-simu_un6A.reg_B_agg + simu_un0.reg_B_agg)/simu_un0.reg_B_agg;
inc7 = (-simu_un7A.reg_B_agg + simu_un0.reg_B_agg)/simu_un0.reg_B_agg;
incN = (-simu_unN.reg_B_agg  + simu_un0.reg_B_agg)/simu_un0.reg_B_agg;

temp = [simu_un0.reg_B_agg, simu_un1A.reg_B_agg, simu_un2A.reg_B_agg, simu_un4A.reg_B_agg, simu_unN.reg_B_agg,simu_un5A.reg_B_agg, simu_un6A.reg_B_agg, simu_un7A.reg_B_agg;     0, inc1, inc2, inc4, incN, inc5, inc6, inc7];
