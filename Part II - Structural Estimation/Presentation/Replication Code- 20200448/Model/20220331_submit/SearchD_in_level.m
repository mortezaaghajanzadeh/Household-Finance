function [D1, rd1, G1, N1, profitD, profitN] = SearchD_in_level(Par,E0,L0,B1,i,rd_bar,D_star,rd_star)
...
profit_matD = ((1-Par.theta)*D_mat*f - D_mat.*rd_mat - Par.cd0*D_mat - Par.cd1*D_mat.^2/2) + repmat(E0*f,Par.nD,1) - Par.fix; 
profit_matN =  Par.a0 *N_mat - Par.a1*N_mat.^2/Par.D_star(i)/2; 

profit_matD = profit_matD*Par.beta;         
profit_matN = profit_matN*Par.beta;         
         
        
[~,index1] = max(profit_matD+profit_matN);
...
profitD = profit_matD(index_mat1);
profitN = profit_matN(index_mat1);
...
profitD  = reshape(profitD, n1, n2);
profitN  = reshape(profitN, n1, n2);
end