function [m,sPi,sW] = simul_counter(paramvec,nSimul,useGPU)
...
param.capital_tax = paramvec(4); % Capital Icnome Tax
...
Wp = (1 - c.Grid).*( exp(w.Grid) + exp(calibr.g*t)*(1 - param.phi.*(pi.Grid>0) ) ).*((pi.Grid.*R.Grid + (1-pi.Grid)*calibr.Rf) * (1- param.capital_tax)); 
