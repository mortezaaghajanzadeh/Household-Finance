...
VarY=reshape(inv(eye(Ny^2)-kron(F,F))*reshape(Q*Q',Ny^2,1),Ny,Ny); %VarY     = disclyap(F, Q * Q');
...
%% TRASH
%    % MV not uncorrelated!
%    probby = mvncdf(endpoints, EX', VarX);
%    probby(probby == Inf) = 0;
%    probby(probby < 0) = 0;
%    [dennis, daisy] = sort(probby);
%    dennis = diff([0; dennis]);
%    dennis(end) = 1 - sum(dennis(1:end-1));
%    probby(daisy) = dennis;
%    p0 = probby;
%    if any(p0 <= 0)
%       warning('elmi:debug', 'check this out')
%       p0
% %       keyboard
%    end
